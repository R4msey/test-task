...
export const Card = ({
  ...
  const ChangeStorage = async (e) => {
    setState(e !== null ? e.target.value : data.status)
    let response = await localforage.getItem('items')

    for (const i in response) {
      if (JSON.stringify(response[i]) === JSON.stringify(data)) {
        if (e == null) {
          response = response.filter((item, index) => index !== Number(i))
        } else {
  ...
  return (
    <div
      className={`wrapper ${data.status === 'Unavailable' ? 'disabled' : null}`}
      style={{ border: `3px solid ${currentColor[0].color}` }}
    >
